    ...
    <main className="bg-white min-h-screen ">
      <section className="py-4 px-4 sm:px-0 sm:max-w-screen-lg lg:max-w-screen-lg xl:max-w-screen-xl mx-auto text-gray-900">
        <section className={`w-full sm:w-3/4 mx-auto relative`}>
          <div className={`absolute bottom-1/4 left-1/4 border-4 border-${data.payment.status === 'SUCCESS' ? 'green-300':'red-300'} w-120 h-32 flex items-center justify-center opacity-40`}> 
            <span className={`text-5xl text-center text-${data.payment.status === 'SUCCESS' ? 'green-300':'red-300'}`}>{data.payment.status === 'SUCCESS' ? 'Sudah':'Belum'} Lunas</span>
          </div>
          <div className="mb-2 flex justify-between items-center">
            <img src={logo} width={100} height={40} alt="DNR" />
            <h1 className="text-base font-semibold text-gray-900">Faktur {data.transaction_number}</h1>
          </div>
