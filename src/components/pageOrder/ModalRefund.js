...
export default function ModalRefund({open, setOpen, order, refetch}) {
  const {mutate, isLoading} = useMutation(
    'refund-payment',
    (payload) =>
      serverAuthAPI({
        url: `/admin/orders/${order.transaction_number}/refund`,
        method: 'POST',
      }),
    {
      onSuccess() {
  ...
  return (
    <ConfirmationModal
      ...
      message="Apakah anda yakin untuk menyelesaikan proses refund pesanan ini"
